    ...
    return (
        <div className="App">
            <header className="App-header">
                ...
                {detailView && <div className="detailView" style={{ background: "#fff", margin:"10px", padding: "25px", boxShadow: "0 0 5px 5px lightgray" }}>
                    <h1>{currObj.name}</h1>
                    <h3>Details</h3>
                    <p style={{ fontSize: "medium" }}>Warehouse details are given below. You can edit them and click on save to modify.</p>
                    <div className="flexBox flex-Mv" >
                        <div>
                            <h5>Name</h5>
                            <input style={{margin:0}} type="text" onKeyUp={(e) => { currObj.name = e.target.value; }} placeholder={currObj.name} className="searchBox" />
                        </div>

                        <div>
                            <h5>City</h5>
                            <input style={{margin:0}} type={"text"} onKeyUp={(e) => { currObj.city = e.target.value }} placeholder={currObj.city} className="searchBox" />
                        </div>
                        <div>
                            <h5>Space available</h5>
                            <input style={{margin:0}} type={"text"} onKeyUp={(e) => { currObj.space_available = e.target.value }} placeholder={currObj.space_available} className="searchBox" />
                        </div>
                    </div>
                    <br />
                    <button style={{width:"fit-content"}} onClick={(e) => { save(currObj.id) }} className="searchBox" >Save Details</button>
                </div>}
            </header>
        </div>
    );
